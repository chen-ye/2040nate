...
//googlePlusOne tracking
function plusoneTrack(obj) {
  var s=s_gi(s_account);
  s.linkTrackVars="eVar57,events";
  s.linkTrackEvents="event31";
  s.events="event31"; 
  s.eVar57="googlePlusOne-share";
  s.tl(this,"o","Google Plus One Share"); 
};

var s=s_gi(s_account);

s.getUrlVars = function(){
  var i = 0;
  var args = new Object();
...
s.getCampaign = function(){
  var urlvars = s.getUrlVars();
  var campaignSrc = '';
  if (typeof urlvars.campaign != "undefined") {
    campaignSrc = urlvars.campaign;
  } 
  return campaignSrc;
};
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,docx,pdf,xls,xlsx,3gp,avi,mpeg,swf,wma,m1v,mp4,flv,rm,divx,ppt,pptx"
s.linkInternalFilters="javascript:,mirrorreaderoffers.co.uk,icnetwork.co.uk,fish4.co.uk,people.co.uk,business7.co.uk,insider.co.uk,karaokechamp.com,mirrorshopping.co.uk,scotwheels.co.uk,scotcareers.co.uk,mirrorfootball.co.uk,youthemanager.co.uk,walesonline.co.uk,liverpoolecho.co.uk,mirrordating.co.uk,dailyrecord.co.uk,3am.co.uk,prideofbritain.com,mirrorcashback.com,mirrorbingo.com,sundaymirror.co.uk,mirror.co.uk,"+location.hostname
...
// this is used to clean the pageName report, ensuring only readable characters are sent
function SCStrip2(a) {
  var len = a.length;
  ...
  for (var i = 0; i <= len; i++) {
    ...
    switch (sChar) { 
      ...
      case "+": 
        outStr.push("plus"); 
        break; 
      case "&": outStr.push("and"); 
        break; 
      default: outStr.push(sChar); 
    }
  }
  return outStr.join('');
}
...
/* Add calls to plugins here */   
  /* Returns page name as lowercase and stripped of non-readable characters */
  s.pageName=SCStrip2(s.pageName.toLowerCase());

  /* Sets hierarchy to be equal to the pageName */
  s.hier1=replaceAll(s.pageName,':','|');
  ...
  s.campaign=s.getValOnce(s.getQueryParam('icid'),'ext_campaign',30);
  
  //Get previous page value
  s.prop55=s.getPreviousValue(s.pageName,'gpv_pn','');
  s.eVar55=s.prop55;

  // Page URL excluding Query Strings
  s.prop54=location.protocol + "//" + location.host + location.pathname;
  s.eVar54=s.prop54;

  /* New vs Return Visits  */ 
  s.eVar48=s.getNewRepeat();  
  ...
  s.eVar49=s.getVisitNum('m');
  if(s.eVar49 && s.eVar49!=""){
    s.prop53=s.eVar49;
    s.events=s.apl(s.events,"event39",",",2);
  }
  
  /* Set Visitor Id dynamically from s_vi cookie */
  s.prop49=s.eVar51="D=s_vi";
  
  /* Setup Dynamic Object IDs */
  s.setupDynamicObjectIDs();  
  s.prop60=s.eVar58="D=oid";
  s.prop61=s.eVar59="D=pid";
  ...
  if(s.prop30 && s.prop30!=""){
    s.prop30=s.prop30.toLowerCase();
    s.eVar43=s.prop30;
  }
  ...
  /*internal search term */
   if(s.prop24 && s.prop24!=""){
    ...
    s.eVar46=getSearchStatus();
    s.events=s.apl(s.events,"event9",",",2);  
  }
   
   /*add status to search result term - 
   failed search = zero results returned in prop25*/
  function getSearchStatus(){
    var prefix = (s.prop25 ==="zero") ? "f" : "r";
    /*capture failed search in event33 */
    if(prefix === "f"){
      s.events=s.apl(s.events,"event33",",",2);
      };
    return prefix + "_" + s.prop24;
  }
  ...
  internalLinkTrackingVars();
};

//quiz tracking
function quizTrackStart() {
  s.linkTrackVars='eVar9,eVar10,eVar11,events,products,prop5,prop16,prop21,prop30,prop72';
  s.linkTrackEvents=s.events='event46';
  s.eVar9=s.prop16;
  s.eVar10=s.prop21;
  s.eVar11=s.prop5;
  s.prop30='quiz';        
  s.prop72='qz:_' + s.prop15 + '_start';
  s.products = 'quiz;' + s.prop15+';'
  s.tl(true,'o','Quiz');
};

function quizTrackNextQ(qIndex) {
  s.linkTrackVars='eVar9,eVar10,eVar11,events,products,prop5,prop16,prop21,prop30,prop72';
  ...
  s.eVar11=s.prop5;
  s.prop30='quiz';
  s.prop72='qz:_' + s.prop15 + '_q'+qIndex;
  s.products = 'quiz;' + s.prop15+';'
  s.tl(true,'o','Quiz');
};

function quizTrackEnd() {
  var s=s_gi(s_account);
  s.linkTrackVars="eVar9,eVar10,eVar11,events,prop5,prop16,prop21,prop30,prop72";
  s.linkTrackEvents="event47,events";
  s.events="event47";
  s.eVar9 = s.prop16;
  s.eVar10 = s.prop21;
  s.eVar11 = s.prop5;
  s.prop30 = "quiz";
  s.prop72 = "qz:_"+ s.prop15 +"_completed";
  s.products = 'quiz;' + s.prop15+';'
  s.tl(true,"o","Quiz");
};

/***
* Needs to be called on page-load - these variable if set are from a link on
* an originating page (e.g. opinion-carousel or other widgets).
***/
function internalLinkTrackingVars(){
  var pageName = sessionStorage.getItem('tm.omniture.linkTracking.pageName');
  var widgetContext = sessionStorage.getItem('tm.omniture.linkTracking.context');
  var dataType = sessionStorage.getItem('tm.omniture.linkTracking.dataType');
  var dataAction = sessionStorage.getItem('tm.omniture.linkTracking.dataAction');
  var friendlyName = sessionStorage.getItem('tm.omniture.linkTracking.friendlyName');  
  var linkNumber = sessionStorage.getItem('tm.omniture.linkTracking.linkNumber');
 
   if (pageName && linkNumber && widgetContext && friendlyName){    
    s.linkTrackEvents = s.events=s.apl(s.events,"event29",",",2);    
    s.prop71 = 'lnk|' + widgetContext + '|' + linkNumber + "|" + dataType + '|' + dataAction;
    s.eVar75 = pageName + '_' + s.prop71;
    }   
  };
...
s.cookieDomainPeriods = (window.location.hostname.split(".").length - 1).toString();
s.visitorNamespace="trinitymirror";
s.trackingServer=s_omnitureDNSAlias;
